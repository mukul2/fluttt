...
Future<dynamic> signupPatient(String name,String phone,String email,String type,String password) async {
  final http.Response response = await http.post(
    _baseUrl + 'sign-up',
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
    body: jsonEncode(<String, String>{'phone': phone,'email': email,'user_type': type,'name': name,'password': password,'department':"0"}),
  );
  //showThisToast(response.statusCode.toString());
  if (response.statusCode == 200) {
...
Future<dynamic> signupDoctor(String name,String phone,String email,String type,String password,String depaertment) async {
  final http.Response response = await http.post(
    _baseUrl + 'sign-up',
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
    body: jsonEncode(<String, String>{'phone': phone,'email': email,'user_type': type,'name': name,'password': password,'department':depaertment!=null&&depaertment.length>0?depaertment:"0"}),
  );
  //showThisToast(response.statusCode.toString());
  if (response.statusCode == 200) {
  ...
  Future<dynamic> performAppointmentSubmit(String AUTH,String patient_id,String dr_id,String problems, String phone, String name ,String chamber_id,String date, String status, String type) async {
    final http.Response response = await http.post(
